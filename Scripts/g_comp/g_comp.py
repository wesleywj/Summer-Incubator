...
def train_and_evaluate_logistic(X, y, label, save_dir="model_outputs"):
    import os
    import numpy as np
    import pandas as pd
    import matplotlib.pyplot as plt
    ...
    from sklearn.pipeline import make_pipeline
    from sklearn.metrics import (
        accuracy_score, roc_auc_score, ConfusionMatrixDisplay, roc_curve
    )
    ...
    X_train, X_test, y_train, y_test = train_test_split(
        X, y_np, test_size=0.3, random_state=42
    )
    ...
    # Save confusion matrix
    disp = ConfusionMatrixDisplay.from_estimator(logreg_model, X_test, y_test)
    plt.title(f"Confusion Matrix: {label}")
    plt.savefig(os.path.join(save_dir, f"{label}_confusion_matrix.png"))
    plt.close()
...
def logistic_coef_summary(model, X, y, label, save_dir="model_outputs"):
    """
    Get coefficients, standard errors, z-values, and p-values for a sklearn LogisticRegression model.
    Drops perfectly collinear variables if needed (mimics R/Stata).
    Saves results as CSV using label for file name.
    """
    ...
    X_scaled = model.named_steps['standardscaler'].transform(X)  # Standardized X
    
    # Predictions
    pred_probs = log_reg.predict_proba(X_scaled)[:, 1]
    ...
    # Try strict inverse
    try:
        cov_logit = inv(X_design.T @ V @ X_design)
        dropped_features = []
    except LinAlgError:
        print("❌ Singular matrix: perfect multicollinearity detected. Dropping variables...")
        
        # Detect collinearity using rank
        rank = np.linalg.matrix_rank(X_design.T @ V @ X_design)
        total = X_design.shape[1]
        
        # Drop variables until matrix is full rank
        dropped_features = []
        keep_idx = list(range(total))
        
        for i in range(1, total):  # try dropping one-by-one (skip intercept at idx=0)
            test_idx = keep_idx.copy()
            test_idx.pop(i)
            test_matrix = X_design[:, test_idx]
            if np.linalg.matrix_rank(test_matrix.T @ V @ test_matrix) == rank:
                dropped_features.append(i)
                keep_idx = test_idx
        
        # Recompute with dropped variables
        X_design = X_design[:, [i for i in range(total) if i not in dropped_features]]
        cov_logit = inv(X_design.T @ V @ X_design)
    
    # Coefficients (include intercept, drop any dropped features)
    all_coefs = np.concatenate([log_reg.intercept_, log_reg.coef_.flatten()])
    if dropped_features:
        coefs = np.delete(all_coefs, dropped_features)
    else:
        coefs = all_coefs
    ...
    # Feature names (drop collinear features if needed)
    if hasattr(X, "columns"):
        feature_names = ["Intercept"] + list(X.columns)
    else:
        feature_names = ["Intercept"] + [f"feature_{i}" for i in range(X.shape[1])]
    if dropped_features:
        feature_names = [f for i, f in enumerate(feature_names) if i not in dropped_features]
        print(f"⚠️ Dropped features due to collinearity: {dropped_features}")
    ...
    # File path using label
    csv_path = os.path.join(save_dir, f"{label}_coef_summary.csv")
    summary_df.to_csv(csv_path, index=False)
    print(f"✅ Saved coefficient summary to: {csv_path}")
...
#     Uses y_cf from `ref_medoid` as the reference and recalculates
#     y_ref each bootstrap replicate to properly propagate uncertainty.
#     """
...
#     # First pass: compute all y_cf values
...
#     # Second pass: compute ATEs relative to reference (recalculating y_ref each time)
#     for medoid_id, y_cf in counterfactuals_y.items():
#         # Recalculate reference distribution inside the loop
#         y_ref = y_dic
#         mean_y_ref =  y_ref.mean()

#         mean_y_cf = np.mean(y_cf)
#         ates[medoid_id] = mean_y_cf - mean_y_ref
#         stand_ates[medoid_id] = (mean_y_cf - mean_y_ref) / mean_y_ref
#         risk_ratios[medoid_id] = mean_y_cf / mean_y_ref

#         print(f"{medoid_id}: ATE={ates[medoid_id]:.3f}, "
#               f"Std ATE={stand_ates[medoid_id]:.3f}, "
#               f"RR={risk_ratios[medoid_id]:.3f}")

#     return counterfactuals_y, counterfactual_X, ates, stand_ates, risk_ratios

##############################################################
## Function 1 : Reference scenario: Full-time with children ##
#############################################################
...
def counterfactual_y_under_X_medoid_features(
    ...
    tv_cov_name="dt_n_years_disease_dic",
    outcome_feature_names=None,
    ref_medoid=2  # full-time with children 
): 
    """
    For each medoid, simulate counterfactual predictions by updating
    the disease features, predicting the covariate, and updating the
    ...
    for medoid_id, y_cf in counterfactuals_y.items():
        # Recalculate reference distribution inside the loop
        y_ref = counterfactuals_y[f"medoid_{ref_medoid}"]
...
def summarize_bootstrap_percentile_ci(df, drop_seed=True, risk_ratio=False):
    """
    Summarize bootstrap results with percentile-based confidence intervals
    and significance stars:
      *    CI does not cross null and alpha=0.05
      **   CI does not cross null and alpha=0.01
      ***  CI does not cross null and alpha=0.001

    Parameters:
    - df (pd.DataFrame): Bootstrap results with columns for each target.
    - drop_seed (bool): Whether to drop the 'seed' column.
    - risk_ratio (bool): If True, null value is 1 (RR); else null is 0 (difference).
    ...
    # Null depends on difference vs RR
    null_value = 1 if risk_ratio else 0

    # Significance stars based on CI crossing null
    def stars(row):
        if row["ci_lower"] > null_value or row["ci_upper"] < null_value:
            # CI excludes null → significant
            # *** if both bounds are far from null (approximate)
            # We keep the same thresholds (but based on interval width)
            ci_width = row["ci_upper"] - row["ci_lower"]
            if ci_width < 0.001:  # very tight
                return "***"
            elif ci_width < 0.01:
                return "**"
            elif ci_width < 0.05:
    ...
    summary["stars"] = summary.apply(stars, axis=1)
